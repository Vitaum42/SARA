...
function renderTabelaPoliticos(deputados) {
  ...
  tbody.innerHTML = deputados.map(dep => {
    // Usa cache do dashboard se disponível; caso contrário exibe "carregando"
    ...
    return `
    <tr data-nome="${dep.nome}" data-partido="${dep.siglaPartido || ''}" data-estado="${dep.siglaUf || ''}" data-status="Ativo">
      <td>
        <div class="pol-name-cell">
          <div class="avatar">${getInitials(dep.nome)}</div>
          <div>
            <div style="font-weight:600">${dep.nome}</div>
            <div style="font-size:.74rem;color:var(--text-light)">${dep.siglaPartido || ''} · ${dep.siglaUf || ''}</div>
          </div>
        </div>
      </td>
      ...
      <td>${dep.siglaUf || '—'}</td>
      <td class="expense-amount" id="gasto-${dep.id}">${gastoHtml}</td>
      <td><span class="badge badge-green">Ativo</span></td>
      <td style="display:flex;gap:6px">
        <button class="action-btn"
          onclick="abrirDetalhes(${dep.id}, '${dep.nome.replace(/'/g, "\\'")}')">Ver</button>
        <button class="action-btn" style="background:#f0fdf4;color:#16a34a;border-color:#bbf7d0"
          onclick="abrirGastosDeputado(${dep.id}, '${dep.nome.replace(/'/g, "\\'")}')">💰 Gastos</button>
      </td>
    </tr>`;
  }).join('');
}
...
async function initAPI() {
  ...
  if (deputados) {
    const ufs = [...new Set(deputados.map(d => d.siglaUf).filter(Boolean))].sort();
    ...
    if (selStatus) {
      selStatus.innerHTML = `<option value="">Todos os Status</option><option value="Ativo">Ativo</option>`;
    }
  }
